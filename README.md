...
npm test               # 231 tests: oracle, invariants, consistency, assets, schema, platform
npm run lint
npm run typecheck
npm run analyze        # per-property accuracy table vs CoolProp
npm run build
npm run verify:bundle  # 44 artifact-integrity checks
npm run e2e            # 124 Playwright tests across all three artifacts
```
...
- **CI on every push**: lint, typecheck, 231 tests (CoolProp oracle, physical
  ...
  checks, and 124 Playwright tests covering all three artifacts — the raw-served
