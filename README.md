...
npm test               # 274 tests: oracle, invariants, consistency, assets, schema, platform
npm run lint
npm run typecheck
npm run analyze        # per-property accuracy table vs CoolProp
npm run build
npm run verify:bundle  # 46 artifact-integrity checks
npm run e2e            # 180 Playwright tests across all three artifacts
```
...
- **CI on every push**: lint, typecheck, 274 tests (CoolProp oracle, physical
