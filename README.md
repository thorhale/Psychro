...
npm test               # 193 tests: oracle, invariants, consistency, assets, schema, platform
npm run lint
npm run typecheck
npm run analyze        # per-property accuracy table vs CoolProp
npm run build
npm run verify:bundle  # 44 artifact-integrity checks
npm run e2e            # 98 Playwright tests across all three artifacts
```
...
- **CI on every push**: lint, typecheck, 193 tests (CoolProp oracle, physical
  ...
  checks, and 98 Playwright tests covering all three artifacts — the raw-served
